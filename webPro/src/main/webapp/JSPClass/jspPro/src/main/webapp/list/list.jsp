...
<%
	...
	int note_idx, view_count, content_idx, genre_idx;
	String title, text, img, create_at, edit_at;
	// Date create_at, edit_at;
	...
	try{
		...
		if(rs.next()){
			...
			do{
				...
				edit_at = rs.getString("edit_at");
				view_count = rs.getInt("view_count");
				content_idx = rs.getInt("content_idx");
				genre_idx = rs.getInt("genre_idx");
				
				vo = new NoteVO(note_idx, title, text, img, create_at, edit_at, view_count, content_idx, genre_idx);
				list.add(vo);
...
<script>
	...
	function loadFullBoardData(maxPage) {
	  ...
	  %>
	  posts.push({ id: <%= list.get(i).getNote_idx() %>, title: "<%= list.get(i).getTitle() %>" });
	  <%
	  }
	  %>
